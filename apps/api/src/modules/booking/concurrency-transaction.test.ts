...
test("I: concurrent overlapping multi-cell bookings leave no partial writes", { skip: !CI_ONLY }, async () => {
  ...
  for (let a = 0; a < survivors.length; a++) {
    for (let b = a + 1; b < survivors.length; b++) {
      ...
      assert.ok(!overlaps, `survivors ${survivors[a]!._id.toHexString()} and ${survivors[b]!._id.toHexString()} overlap`);
    }
  }
  ...
  const claims = await collections
    ...
    .find({ venueId: venue.venueId, stationId: venue.stationIds[0]!, status: "confirmed" })
    .toArray();
  const expectedClaims = survivors.reduce((sum, booking) => sum + booking.slotCount + booking.bufferSlotCount, 0);
  assert.equal(claims.length, expectedClaims, "confirmed claim count must equal the sum of survivors' cells");
