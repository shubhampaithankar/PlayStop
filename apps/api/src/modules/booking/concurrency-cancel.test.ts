...
test(
  "K2: concurrent double cancel returns 200 everywhere with one cancelledAt",
  { skip: !CI_ONLY },
  async () => {
    ...
    // Cleanup runs in finally so a failed assertion still closes the server,
    // Mongo pool, and ioredis client instead of leaving them open: an
    // unclosed ioredis client keeps retrying forever (its retryStrategy never
    // returns null), which keeps the event loop alive and hangs the file.
    try {
