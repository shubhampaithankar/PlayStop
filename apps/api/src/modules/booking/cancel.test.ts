...
test("cancel routes", async (t) => {
  ...
  try {
    ...
    await t.test("cancel frees the cells for a fresh booking", async () => {
      ...
      const targetCells = availBeforeBody.cells.filter((c) =>
        cellStartMs.includes(new Date(c.startsAt).getTime()),
      );
      ...
      const targetCellsAfter = availAfterBody.cells.filter((c) =>
        cellStartMs.includes(new Date(c.startsAt).getTime()),
      );
      assert.ok(targetCellsAfter.every((c) => c.state === "free"));

      const rebooked = await confirm({
        stationId,
        startsAt,
        slotCount: 3,
        partySize: 1,
    ...
    await t.test("a booking that already started is 422 BOOKING_NOT_CANCELLABLE", async () => {
      ...
      await collections
        .bookings()
        .updateOne(
          { _id: new ObjectId(bookedBody.id) },
