...
test("A: N concurrent confirms yield exactly one booking", { skip: !CI_ONLY }, async () => {
  ...
  for (let round = 0; round < ROUNDS; round++) {
    ...
    assert.ok(startSpreadMs < 1000, `round ${round}: requests not observed to overlap (${startSpreadMs}ms spread)`);
    ...
    assert.equal(winners.length, 1, `round ${round}: expected exactly one 201, got ${winners.length}`);
    assert.equal(conflicts.length, N - 1, `round ${round}: expected ${N - 1} SLOT_TAKEN, got ${conflicts.length}`);
    assert.equal(timeouts.length, 0, `round ${round}: expected zero BOOKING_TIMEOUT, got ${timeouts.length}`);
...
test("D: concurrent identical-key retries yield exactly one booking", { skip: !CI_ONLY }, async () => {
  const { collections } = await import("#libs/mongo/index.js");
  const { closeTestResources, fireBurst, futureSessionCells, seedVenue, startTestServer, wipeVenue } = await import(
    "#testing-support.js"
  );
  ...
  for (const c of classified) {
    ...
    assert.notEqual(c.code, "SLOT_TAKEN", "the idempotency layer let a duplicate reach the insert");
  }
  assert.ok(classified.some((c) => c.status === 201), "no request ever won or replayed");
