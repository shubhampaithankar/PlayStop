...
test("hold routes", async (t) => {
  ...
  try {
    ...
    await t.test(
      "slotCount outside the station's range is 422 SLOT_COUNT_OUT_OF_RANGE",
      async () => {
    ...
    await t.test("an overlapping hold acquires nothing on failure", async () => {
      ...
      await releaseHold({
        holdId: holdFreeBody.holdId,
        stationId,
        startsAt: startsAtFree,
        slotCount: 2,
      });
    });
    ...
    await t.test("release only deletes cells this holder still owns", async () => {
      ...
      const staleRelease = await releaseHold({
        holdId: holdABody.holdId,
        stationId,
        startsAt,
        slotCount: 1,
      });
