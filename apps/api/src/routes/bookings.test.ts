...
async function confirm(body: unknown, idempotencyKey: string | null = randomUUID()): Promise<Response> {
...
test("booking confirm and read routes", async (t) => {
  ...
  await t.test("missing Idempotency-Key is 400 IDEMPOTENCY_KEY_REQUIRED", async () => {
    ...
    const res = await confirm(
      { stationId, startsAt: new Date(cellStartMs[0]!).toISOString(), slotCount: 1, partySize: 1, player: player() },
      null,
    );
  ...
  await t.test("a malformed body is 400 VALIDATION_FAILED", async () => {
    const res = await confirm({ stationId, startsAt: "not-a-date", slotCount: 1, partySize: 1, player: player() });
    assert.equal(res.status, 400);
    const body = (await res.json()) as { error: { code: string } };
  ...
  await t.test("slotCount over the station's maxSlots is 422 SLOT_COUNT_OUT_OF_RANGE", async () => {
    const { cellStartMs } = futureSessionCells(venue, 1);
    const res = await confirm({
      stationId,
      startsAt: new Date(cellStartMs[0]!).toISOString(),
  ...
  await t.test("a maintenance window overlapping the range is 409 SLOT_UNAVAILABLE", async () => {
    ...
    await collections.stations().updateOne(
      ...
      { $set: { maintenanceWindows: [{ startsAt: new Date(cellStart), endsAt: new Date(cellStart + 30 * 60_000) }] } },
    );
    const res = await confirm({
      stationId,
    ...
    await collections.stations().updateOne({ _id: venue.stationIds[0]! }, { $set: { maintenanceWindows: [] } });
  });

  await t.test("booking an already-confirmed cell is 409 SLOT_TAKEN", async () => {
    const { cellStartMs } = futureSessionCells(venue, 1);
    const startsAt = new Date(cellStartMs[0]!).toISOString();
    const first = await confirm({ stationId, startsAt, slotCount: 1, partySize: 1, player: player() });
    assert.equal(first.status, 201);

    const second = await confirm({ stationId, startsAt, slotCount: 1, partySize: 1, player: player() });
  ...
  await t.test("confirming with an expired holdId is 410, then succeeds without one", async () => {
    const { cellStartMs } = futureSessionCells(venue, 1);
    const startsAt = new Date(cellStartMs[0]!).toISOString();
    const hold = await fetch(`${server.baseUrl}/v1/venues/${venue.slug}/holds`, {
      method: "POST",
    ...
    const withoutHold = await confirm({ stationId, startsAt, slotCount: 1, partySize: 1, player: player() });
  ...
  await t.test("a request racing an in-flight claim gets 409 REQUEST_IN_FLIGHT", async () => {
    ...
    const body = { stationId, startsAt: "2026-08-07T08:30:00.000Z", slotCount: 1, partySize: 1, player: player() };
  ...
  await t.test("a partial multi-cell booking is impossible (loses on the middle cell)", async () => {
    const { cellStartMs } = futureSessionCells(venue, 3);
    ...
    assert.equal(
      await collections.slotClaims().countDocuments({ venueId: venue.venueId, stationId: venue.stationIds[0]!, cellStart: new Date(s1) }),
      0,
    );
    assert.equal(
      await collections.slotClaims().countDocuments({ venueId: venue.venueId, stationId: venue.stationIds[0]!, cellStart: new Date(s3) }),
      0,
    );
    assert.equal(
    ...
    assert.equal(
      await collections
        .bookings()
        .countDocuments({ venueId: venue.venueId, stationId: venue.stationIds[0]!, startsAt: new Date(s1) }),
      0,
    );
  });
  ...
  await t.test("reading a booking with the wrong code is 404 BOOKING_NOT_FOUND", async () => {
    ...
    const body = (await res.json()) as BookingResponse;
    const read = await fetch(`${server.baseUrl}/v1/venues/${venue.slug}/bookings/${body.id}?code=ZZZZZZZZZZ`);
    assert.equal(read.status, 404);
    const readBody = (await read.json()) as { error: { code: string } };
    assert.equal(readBody.error.code, "BOOKING_NOT_FOUND");
  });

  await t.test("cross-tenant: a station id from venue B is 404 under venue A's slug", async () => {
